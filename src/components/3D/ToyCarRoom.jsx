...
export default function ToyCarRoom({ viewState }) {
  ...
  /* The listeners below are bound to window for the life of the component, so
     they need to know the current view without re-binding on every change. */
  const viewRef = useRef(viewState)
  viewRef.current = viewState

  /* Whether the car should be listening at all.

     It should not, in two cases that this was missing entirely:

     · SOMETHING IS FOCUSED. These are single-letter shortcuts on window, so
       typing into the Contact form or Spotlight was driving the car — "h"
       sounded the horn mid-sentence, "l" flipped the headlights, and "r" reset
       the car and rebuilt every block on the rug. Searching for "rail" did
       three of those in a row.
     · WE ARE NOT IN THE ROOM. useFrame already refused to *move* the car when
       zoomed into the monitor, but the handlers still ran, so playing Snake
       with WASD steered the car underneath and honked on H. */
  const carKeysActive = () => {
    if (viewRef.current && viewRef.current !== 'ROOM') return false
  ...
  useEffect(() => {
    ...
    /* Keyup is deliberately NOT gated: a key pressed while the car was
       listening must always be released, or it sticks down. Same reason for
       the blur reset — alt-tabbing away mid-corner never delivers the keyup,
       and the car would drive itself into the wall on return. */
    const releaseAll = () => {
      const k = keys.current
      k.forward = k.backward = k.left = k.right = k.nitro = false
    }
