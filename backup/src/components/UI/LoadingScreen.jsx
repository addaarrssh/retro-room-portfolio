  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex w-[min(90vw,520px)] flex-col items-center px-6 text-center">
        ...
        {/* Enter gate. Also the user gesture that lets audio start at all. */}
        <button
          type="button"
          onClick={handleEnter}
          disabled={!complete}
          className={`group mt-10 flex items-center gap-3 rounded-full border px-7 py-3 font-mono text-xs uppercase tracking-[0.2em] transition-all duration-300 ${
            complete
              ? 'cursor-pointer border-cyan-400/40 bg-cyan-400/10 text-cyan-200 hover:border-cyan-300 hover:bg-cyan-400/20 hover:tracking-[0.26em] active:scale-95'
              : 'cursor-default border-white/5 text-zinc-700'
          }`}
        >
          <span>{complete ? 'Enter the room' : 'Loading'}</span>
          <ArrowRight
            size={14}
            className={complete ? 'transition-transform group-hover:translate-x-1' : 'opacity-30'}
          />
        </button>

        <p className="mt-6 font-mono text-[10px] leading-relaxed text-zinc-600">
          Best with sound on · Drag to look around · Click the monitor to sit down
        </p>
      </div>
    </div>
  )
