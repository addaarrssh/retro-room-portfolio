  ...
  return (
    <PaletteContext.Provider value={palette}>
      <group>
        ...
        <Door />
        <CeilingLight appearance={appearance} onToggleAppearance={onToggleAppearance} />
        <MovingSunlight />
        <Rug />
        <Desk />
        ...
        <DeskClutter era={era} onToggleEra={onToggleEra} onHoverProp={onHoverProp} />
        <DiskRack enabled={canInteract} onOpenApp={onOpenApp} onHoverProp={onHoverProp} />
...
function Desk() {
  ...
  return (
    <group position={[0, 0, DESK_Z]}>
      {/* Top slab - Smooth Rounded Dark Walnut with soft chamfered edges and coffee wear */}
      <RoundedBox args={[DESK_W, 0.06, DESK_D]} radius={0.009} smoothness={5} position={[0, DESK_TOP - 0.03, 0]} castShadow receiveShadow>
        <meshPhysicalMaterial map={wood} roughnessMap={deskWear} color={P.deskTop} {...surfaceWithMaps('lacqueredWood')} />
      </RoundedBox>
...
function Chair({ viewState }) {
  ...
  /* One cushion shape for all seven upholstered panels — they differ only in
     scale, so sharing it keeps the chair at one geometry instead of seven. */
  const cushion = useMemo(() => cushionGeometry({ crown: 0.32 }), [])
  ...
  return (
    <group ref={group} position={[pose.x, 0, pose.z]} rotation={[0, pose.ry, 0]}>
        ...
        {[-1, 1].map((sx) => (
          <group key={sx} position={[sx * 0.225, 0.035, -0.02]} rotation={[0, 0, sx * -0.34]}>
            <mesh rotation={[Math.PI / 2, 0, 0]} castShadow>
              <capsuleGeometry args={[0.048, 0.3, 12, 28]} />
              <meshPhysicalMaterial color={shell} {...vinyl} />
            </mesh>
      ...
      <group position={[0, 0.93, 0.2]} rotation={[0.1, 0, 0]}>
        ...
        <mesh scale={[0.205, 0.23, 0.05]} position={[0, -0.16, 0]} castShadow geometry={cushion}>
                    <meshPhysicalMaterial color={shell} {...vinyl} />
        </mesh>
        <mesh scale={[0.235, 0.18, 0.052]} position={[0, 0.16, 0]} castShadow geometry={cushion}>
                    <meshPhysicalMaterial color={shell} {...vinyl} />
        </mesh>
        {/* Crown */}
        <mesh scale={[0.172, 0.08, 0.046]} position={[0, 0.37, 0]} castShadow geometry={cushion}>
                    <meshPhysicalMaterial color={shell} {...vinyl} />
        </mesh>
        ...
        {[-1, 1].map((sx) => (
          <group key={sx} position={[sx * 0.235, 0.16, -0.03]} rotation={[0, sx * 0.42, sx * -0.05]}>
            <mesh castShadow>
              <capsuleGeometry args={[0.042, 0.24, 10, 24]} />
              <meshPhysicalMaterial color={shell} {...vinyl} />
            </mesh>
            <RoundedBox
              args={[0.02, 0.3, 0.02]}
        ...
        <group position={[0, 0.3, -0.07]} rotation={[0.08, 0, 0]}>
          <mesh scale={[0.12, 0.06, 0.037]} castShadow geometry={cushion}>
                        <meshPhysicalMaterial color={shell} {...vinyl} />
          </mesh>
          {[-1, 1].map((sx) => (
            <RoundedBox
        ...
        <group position={[0, -0.28, -0.075]} rotation={[-0.06, 0, 0]}>
          <mesh scale={[0.152, 0.07, 0.04]} castShadow geometry={cushion}>
                        <meshPhysicalMaterial color={shell} {...vinyl} />
          </mesh>
          {[-1, 1].map((sx) => (
            <RoundedBox
      ...
      <group rotation={[-0.045, 0, 0]}>
        ...
        >
          ...
          {standardKeys.map((k, i) => (
            <Instance key={i} position={[k.x, 0.022, k.z]} color={k.accent ? '#f43f5e' : '#27272a'} />
          ))}
        </Instances>
...
function DeskClutter({ era = 'MODERN', onToggleEra, onHoverProp }) {
  const mug = useMemo(() => mugProfile(), [])
  const ceramic = surfaceWithMaps('ceramic')

  return (
    <group>
      {/* 9.2 Physical Era Rocker Switch on desk */}
      <group
        position={[0.9, DESK_TOP + 0.008, 0.22]}
        onClick={(e) => {
          e.stopPropagation()
          onToggleEra?.()
        }}
        onPointerOver={() => onHoverProp?.({ label: 'Era Switch', hint: `Current: ${era}. Click to switch to ${era === 'MODERN' ? 'RETRO 1995 (Win95 CRT)' : 'MODERN (Apple Studio)'}` })}
        onPointerOut={() => onHoverProp?.(null)}
      >
        <RoundedBox args={[0.07, 0.016, 0.05]} radius={0.003} smoothness={4} castShadow receiveShadow>
          <meshStandardMaterial color="#2b2d35" roughness={0.4} />
        </RoundedBox>
        <RoundedBox position={[0, 0.01, 0]} rotation={[era === 'MODERN' ? 0.2 : -0.2, 0, 0]} args={[0.045, 0.01, 0.03]} radius={0.002} smoothness={4}>
        <meshStandardMaterial color={era === 'MODERN' ? '#38bdf8' : '#f59e0b'} roughness={0.3} />
      </RoundedBox>
      </group>
  ...
  return (
    ...
    >
      ...
      <group position={[0.01, 0.26, 0.02]}>
        ...
        <meshStandardMaterial color="#1b4d2e" roughness={0.4} />
      </RoundedBox>
...
function Frame({ w, h, print, tint = '#1a1a1e' }) {
  ...
  return (
    <group>
      ...
      {/* 2.5 Peeled poster corner quad rotated ~20° off wall plane */}
      <mesh position={[w / 2 - t * 1.6 - 0.012, h / 2 - t * 1.6 - 0.012, 0.013]} rotation={[0.35, -0.2, 0.1]}>
        <planeGeometry args={[0.025, 0.025]} />
        <meshStandardMaterial color="#f0ede6" roughness={0.88} side={DoubleSide} />
      </mesh>
    </group>
  )
}
...
function Bookshelf({ enabled, onOpenApp, onHoverProp }) {
  ...
  /* One rounded-spine geometry shared by every book on the shelf. The page
     blocks stay square boxes on purpose — a cut page block IS square, and
     rounding it would lose the crisp edge the spine curve plays against. */
  const spine = useMemo(() => bookGeometry(), [])
  ...
  return (
    ...
    >
      ...
      {/* Books, drawn as three instanced groups — spines, page blocks and
          ...
          has to redo. */}
      <Instances limit={60} range={flatBooks.length} geometry={spine} castShadow={false} receiveShadow>
        {/* Cloth-bound boards: a little sheen at grazing angles, no specular
            hotspot. Plain roughness made forty books read as forty erasers. */}
        <meshPhysicalMaterial roughness={0.68} metalness={0} sheen={0.4} sheenRoughness={0.8} />
        {flatBooks.map((b, i) => (
          <Instance
            key={i}
...
function Door() {
  return (
    <group position={[-ROOM.halfX, 0, -0.6]}>
      {/* Frame reveal */}
      <RoundedBox args={[0.08, 2.15, 1.02]} radius={0.008} smoothness={4} position={[0.04, 1.075, 0]}>
        <meshStandardMaterial color="#2d303a" roughness={0.5} />
      </RoundedBox>
      {/* Light spill plane behind door */}
      <mesh position={[-0.1, 1.05, 0]} rotation={[0, Math.PI / 2, 0]}>
        <planeGeometry args={[1.2, 2.2]} />
        <meshBasicMaterial color="#f5e9d6" toneMapped={false} />
      </mesh>
      {/* Light spill onto room floor */}
      <pointLight
        position={[-0.2, 1.2, 0]}
        color="#f5e9d6"
        intensity={0.6}
        ...
        castShadow={false}
      />
      {/* Door Slab — rotated 22 deg open */}
      <group position={[0.04, 0, -0.45]} rotation={[0, 0.38, 0]}>
        <RoundedBox args={[0.045, 2.05, 0.9]} radius={0.01} smoothness={4} position={[0, 1.025, 0.45]} castShadow receiveShadow>
          <meshPhysicalMaterial color="#dedade" roughness={0.35} metalness={0.1} />
        </RoundedBox>
        {/* Handle */}
        <group position={[0.03, 1.0, 0.8]}>
          <mesh rotation={[0, 0, Math.PI / 2]}>
            <cylinderGeometry args={[0.012, 0.012, 0.04, 16]} />
            <meshStandardMaterial color="#b5b8c0" roughness={0.2} metalness={0.9} />
          </mesh>
...
function CeilingLight({ appearance, onToggleAppearance }) {
  const shade = useMemo(() => lampShadeProfile(), [])
  const [hovered, setHovered] = useState(false)
  ...
  return (
    <group
      position={[0, ROOM.height - 0.06, 0.4]}
      onPointerOver={(e) => {
        e.stopPropagation()
        setHovered(true)
      }}
    ...
    >
      ...
      <mesh scale={[1, -1, 1]} castShadow>
        <latheGeometry args={[shade, 32]} />
        <meshStandardMaterial color="#33363f" roughness={0.4} metalness={0.6} side={DoubleSide} />
      </mesh>
      <mesh position={[0, -0.06, 0]}>
        ...
        <meshStandardMaterial
          color="#fff4e0"
          emissive="#ffeecc"
          emissiveIntensity={appearance === 'OVERHEAD' ? 2.5 : 0.8}
          toneMapped={false}
        />
      </mesh>
    </group>
...
function MovingSunlight() {
  const P = usePalette()
  const isLight = P.id === 'LIGHT' || P.id === 'OVERHEAD'
  const lightQuadRef = useRef()

  useFrame(() => {
    if (!lightQuadRef.current) return
    const now = new Date()
    const hours = now.getHours() + now.getMinutes() / 60
    const norm = Math.max(0, Math.min(1, (hours - 6) / 12))
    const targetX = (norm - 0.5) * 4.4
    lightQuadRef.current.position.x = targetX
  })
