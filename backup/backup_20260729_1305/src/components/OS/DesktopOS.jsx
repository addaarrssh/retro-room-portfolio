...
import BootSequence, { BOOT_MS } from './BootSequence'
...
const APPS = [
  ...
  },
  {
    ...
    via: '#2c313c',
    to: '#15181e',
    tint: 'rgba(30,34,42,0.5)',
    label: 'Terminal',
    kind: 'CLI Shell',
    title: 'Terminal.app',
    icon: Terminal,
    gradient: 'from-slate-700 via-zinc-800 to-black shadow-slate-700/50',
    width: 620,
    height: 420,
...
export default function DesktopOS({
  ...
  const T = osThemeFor(appearance, era)
  ...
  /* The wake sequence runs once per mount, and this component mounts when the
     camera starts its flight to the monitor — so the boot plays during the
     approach and the desktop is finished by the time you arrive. Flying back
     out to the room unmounts it, so the next visit boots again. */
  const [woken, setWoken] = useState(false)
  const [hoveredDock, setHoveredDock] = useState(null)

  /* The chrome's own guarantee that it will appear. The boot layer normally
     calls onDone, but it is animation-driven, and the desktop must never be
     left hidden because an animation did not run. Two independent clocks, and
     whichever arrives first reveals the bar, the icons and the dock. */
  useEffect(() => {
    const t = setTimeout(() => setWoken(true), BOOT_MS)
  ...
  useEffect(() => {
    ...
    const resetIdle = () => {
      ...
      idleTimer = setTimeout(() => setScreensaverActive(true), 60000)
    }
  ...
  return (
    <OSThemeContext.Provider value={T}>
      <div
        className={`relative select-none overflow-hidden text-sm antialiased font-sans ${
          era === 'RETRO' ? 'crt-scanlines crt-vignette' : ''
        }`}
      ...
      >
        ...
        <BootSequence run onDone={() => setWoken(true)} />
        {/* 9.6 Retro BIOS POST Boot Sequence */}
        {booting && (
          <div className="absolute inset-0 z-[200] bg-black p-8 font-mono text-xs text-lime-400 space-y-3">
            <div className="text-white font-bold">Award Modular BIOS v4.51PG, An Energy Star Ally</div>
            <div>Copyright (C) 1984-95, Award Software, Inc.</div>
            <div className="pt-4">PENTIUM PRO CPU at 133MHz</div>
            <div>Memory Test : 32768K OK</div>
            <div className="pt-4 space-y-1">
              <div>Detecting PYTHON ............ OK</div>
              <div>Detecting PYTORCH ........... learning</div>
              <div>Detecting SLEEP ............. not found</div>
            </div>
            <div className="pt-8 text-zinc-500">Starting Windows 95...</div>
          </div>
        )}
      ...
      <div
        className="absolute top-12 left-5 z-10 flex flex-col gap-4"
        style={enter(woken, 90, 'translateY(8px)')}
      >
        {APPS.map((app) => {
          const Icon = app.icon
          return (
            <button
              ...
              data-click-sound
              onClick={() => open(app)}
              className="group flex w-[74px] cursor-pointer flex-col items-center gap-1 rounded-xl p-1.5 text-center transition hover:bg-white/10"
            >
              <span className="transition-transform duration-150 ease-out group-hover:-translate-y-2 group-hover:scale-[1.16] group-active:scale-100">
                <AppIcon appId={app.id} from={app.from} via={app.via} to={app.to} tint={app.tint} size={46}>
                  <Icon size={22} strokeWidth={1.9} />
                </AppIcon>
              </span>
              <span className="max-w-full text-[10px] font-semibold leading-tight text-white drop-shadow-[0_1px_3px_rgba(0,0,0,0.9)]">
                {app.label}
              </span>
            </button>
          )
      ...
      <div
        className="absolute inset-x-0 bottom-0 z-50 flex justify-center pb-2.5"
        style={enter(woken, 150, 'translateY(140%)')}
      >
        <div
          onMouseLeave={() => setHoveredDock(null)}
          className={`flex items-end gap-1.5 rounded-2xl border px-2 py-1.5 backdrop-blur-2xl ${T.dock}`}
          style={{
            boxShadow: [
              'inset 0 1px 0 rgba(255,255,255,0.28)', // lit top lip of the shelf
              'inset 0 -1px 0 rgba(0,0,0,0.18)',
              '0 10px 34px rgba(0,0,0,0.34)', // it floats, so the cast is deep
        ...
        >
          {APPS.map((app, i) => {
            ...
            const mag = magnify(hoveredDock, i)
            return (
              <button
                key={app.id}
                data-click-sound
              ...
              >
                <span
                  ...
                  style={{
                    transform: `translateY(${mag.lift}px) scale(${mag.scale})`,
                    transition: 'transform 190ms cubic-bezier(0.2, 0.9, 0.25, 1)',
                  }}
                >
                  <AppIcon appId={app.id} from={app.from} via={app.via} to={app.to} tint={app.tint} size={30}>
...
/* Dock magnification. The icon under the pointer grows and its NEIGHBOURS grow
   too, by a falling amount — that spill onto the neighbours is the entire
   effect. A dock where only the hovered icon scales is a row of buttons with a
   hover state; a dock where the row swells around the pointer is the thing
   people recognise. Gaussian falloff over ~2 icons, which is roughly what the
   real one uses, and a matching lift so the icons stay seated on the shelf
   instead of growing through it. */
const DOCK_AMP = 0.55
const DOCK_SPREAD = 1.6

function magnify(hovered, index) {
  if (hovered === null) return { scale: 1, lift: 0 }
  const d = index - hovered
  const falloff = Math.exp(-(d * d) / (2 * DOCK_SPREAD * DOCK_SPREAD))
  const scale = 1 + DOCK_AMP * falloff
  // Half the extra height, so growth is centred on the icon's own baseline.
  return { scale, lift: -(scale - 1) * 11 }
}

/* Entrance for the desktop chrome once the boot layer hands over. Each piece
   comes from where it lives — the bar from above the top edge, the dock from
   below the bottom — because chrome that fades in from nowhere reads as a web
   page, and chrome that slides in from its own edge reads as an OS. The
   overshoot curve is the standard macOS dock ease. */
function enter(woken, delay, from) {
  return {
    opacity: woken ? 1 : 0,
    transform: woken ? 'none' : from,
    transition: `opacity 320ms ease-out ${delay}ms, transform 520ms cubic-bezier(0.16, 1.06, 0.3, 1) ${delay}ms`,
  }
}
