  ...
  return (
    <div className="h-full w-full overflow-y-auto p-6 font-mono text-sm leading-relaxed text-slate-300 select-text">
      ...
      <div className="mb-6 rounded-lg border border-emerald-500/30 bg-emerald-950/20 p-4 backdrop-blur-sm">
        ...
        <p className="text-xs text-slate-300 mt-2">{profile.positioning}</p>
      </div>

      {/* Stats Grid */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {stats.map((s, i) => (
          <div key={i} className="rounded border border-zinc-800 bg-zinc-900/60 p-3 text-center">
      ...
      {/* Education */}
      <div className="mb-6 rounded border border-zinc-800 bg-zinc-900/40 p-4">
        <div className="flex items-center gap-2 text-emerald-400 font-bold text-xs uppercase mb-2">
          <GraduationCap size={16} />
          <span>Education</span>
        </div>
        <div className="text-sm font-semibold text-slate-100">{education.institution}</div>
        <div className="text-xs text-slate-400">{education.degree} ({education.startYear} - {education.endYear})</div>
        <div className="mt-2 text-xs text-amber-400 font-medium">CGPA: {education.cgpa}</div>
        <p className="mt-2 text-[11px] text-slate-400 italic">{education.note}</p>
      </div>
      ...
      <div className="mb-6">
        ...
        <div className="grid gap-4">
          {skillGroups.map((group) => (
            <div key={group.id} className="rounded border border-zinc-800 bg-zinc-900/40 p-3">
              <div className="text-xs font-bold text-slate-200 mb-2">{group.label}</div>
        ...
        <div className="grid gap-3">
          {principles.map((p, i) => (
            <div key={i} className="rounded border border-zinc-800 bg-zinc-900/40 p-3">
              ...
              <div className="text-[11px] text-slate-400 leading-relaxed">{p.body}</div>
            </div>
          ))}
        </div>
      </div>
