  ...
  const open = useCallback(
    (app) => {
      ...
      setWindows((prev) => {
        ...
        const y = Math.max(10, Math.min(SCREEN_H - TASKBAR_H - h - 10, Math.round((SCREEN_H - TASKBAR_H - h) / 2) + n * CASCADE_STEP))
    ...
    setWindows((prev) =>
      prev.map((w) => {
        ...
        return {
          ...
          y: Math.max(0, Math.min(SCREEN_H - TASKBAR_H - 26, y)),
        }
      }),
    )
  }, [])
    ...
    setWindows((prev) =>
      prev.map((win) => {
        ...
        return {
          ...
          h: Math.max(MIN_H, Math.min(SCREEN_H - TASKBAR_H - win.y, h)),
        }
      }),
    )
  }, [])
