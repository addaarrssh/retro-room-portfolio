...
export default function ProjectsApp({ deepLink }) {
  const T = useOSTheme()
  const [selectedId, setSelectedId] = useState(projects[0].id)
  const project = projects.find((p) => p.id === selectedId) ?? projects[0]

  // Opened by clicking its poster in the room — jump straight to that project.
  useEffect(() => {
    if (deepLink && projects.some((p) => p.id === deepLink)) setSelectedId(deepLink)
  }, [deepLink])
