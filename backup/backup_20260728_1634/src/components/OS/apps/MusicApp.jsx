...
export default function MusicApp() {
  ...
  const categories = [
    ...
    { id: 'INDIAN', label: '🇮🇳 Indian Hits' },
    { id: 'ENGLISH', label: '🇬🇧 English Hits' },
    { id: 'GHAZAL', label: '📜 Classic Ghazals' },
  ]

  const filteredTracks = activeCategory === 'ALL'
    ? tracks.map((t, index) => ({ ...t, originalIndex: index }))
    : tracks.map((t, index) => ({ ...t, originalIndex: index })).filter((t) => t.category === activeCategory)
  ...
  return (
    <div className="h-full w-full overflow-y-auto p-6 text-[13px] leading-relaxed opacity-90 select-text flex flex-col justify-between">
      <div>
        ...
        <div className="mb-4 rounded-xl border border-purple-500/30 bg-purple-950/20 p-5 text-center backdrop-blur-sm relative overflow-hidden">
          <div className="relative z-10">
            <div className="mx-auto mb-3 flex h-20 w-20 items-center justify-center rounded-full bg-current/[0.06] border-4 border-current/15 shadow-2xl relative">
              <div
                className={`h-7 w-7 rounded-full bg-purple-600 border-2 border-purple-300 flex items-center justify-center ${
          ...
          {/* Live analyser bars */}
          <div className="mt-3 flex items-end justify-center gap-1.5 h-7">
          ...
          <button
            ...
            className={`rounded-full px-3 py-1 text-[11px] font-semibold whitespace-nowrap transition ${
              activeCategory === cat.id
                ...
                : 'bg-current/[0.06] opacity-70 hover:opacity-100'
            }`}
          >
            {cat.label}
          </button>
      ...
      {/* Playlist Tracks */}
      <div className="rounded-lg border border-current/10 bg-current/[0.04] p-3 max-h-48 overflow-y-auto">
        <div className="text-xs font-bold opacity-60 uppercase mb-2 flex items-center justify-between">
          <span>Track Listing ({filteredTracks.length})</span>
          <span className="text-[10px] text-purple-400 font-mono">{activeCategory}</span>
        </div>
          ...
          {filteredTracks.map((track) => (
            <button
              ...
              className={`flex w-full items-center justify-between rounded px-3 py-2 text-xs transition ${
                track.originalIndex === currentTrackIndex
            ...
            >
              ...
              <span className="text-[10px] opacity-50 shrink-0">{track.bpm} BPM</span>
            </button>
          ))}
        </div>
      </div>
